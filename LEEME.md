...
1. Descarga esta carpeta completa y pegala donde quieras (no hace falta que este
   dentro de la carpeta del juego).
2. Abre una terminal en esta carpeta y ejecuta una vez:
   ```
   pip install -r requirements.txt
   ```
3. Haz doble clic en **`instalar.bat`** (o ejecuta `python instalar.py` desde la terminal).
